...
public class ClientApplication {
    ...
    private Map doGetProductOne(Map jwtToken) {
        ...
        headers.add("Authorization",
                    "Bearer ".concat((String) jwtToken.get("jwt")));
        HttpEntity requestWithHeaders = new HttpEntity<>(headers);
        String url = "http://my-app:8090/products/1";
        ResponseEntity<Map> product = restTemplate().exchange(url,
                                                              HttpMethod.GET,
                                                              requestWithHeaders,
                                                              Map.class);
    ...
    private Map doAuthenticate(LoginRequest loginReq) {
        String url = "http://my-app:8090/auth/login";

        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);

        HttpEntity<LoginRequest> requestEntity = new HttpEntity<>(loginReq,
                                                                  headers);
    ...
    public RestTemplate restTemplate() {
        ...
        ClientHttpRequestInterceptor interceptor = (request, body, execution) -> {
            ...
            headers.set("Origin",
                        "http://client"); // Cambia esto con tu origen permitido
